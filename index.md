---
type: index
status: active
created: 2026-06-23
updated: 2026-06-23
tags:
  - wiki/index
---

# Index

This is the content map for the vault. Future agents should read this file
before answering questions or editing wiki pages.

## Start Here

- [[README]] - vault overview.
- [[AGENTS]] - operating schema for agents.
- [[log]] - chronological activity log.
- [[operations/ingest-source]] - workflow for adding sources.
- [[operations/query-wiki]] - workflow for answering questions.
- [[operations/lint-wiki]] - workflow for maintaining the graph.
- [[operations/promote-chat-answer]] - workflow for saving reusable answers.

## Core Architecture

- [[raw/README]] - source-of-truth layer.
- [[wiki/README]] - LLM-maintained knowledge layer.
- [[templates/README]] - reusable page templates.
- [[operations/README]] - repeatable wiki workflows.

## Wiki Maps

- [[wiki/maps/home]] - home map for the second brain.
- [[wiki/maps/llm-wiki-architecture]] - explanation of this vault structure.

## Areas

No area pages yet.

## Projects

No project pages yet.

## Topics

- [[wiki/topics/second-brain]] - personal knowledge system overview.
- [[wiki/topics/llm-wiki]] - LLM-maintained wiki pattern.

## Concepts

- [[wiki/concepts/compiled-knowledge]] - knowledge that compounds through maintained structure.
- [[wiki/concepts/source-of-truth]] - immutable evidence layer.
- [[wiki/concepts/wiki-as-codebase]] - treating Obsidian pages as maintained artifacts.

## Entities

No entity pages yet.

## People

- [[wiki/people/andrej-karpathy]] - source author of the referenced LLM Wiki idea.

## Sources

- [[wiki/sources/karpathy-llm-wiki-gist]] - source brief for the LLM Wiki gist.

## Syntheses

- [[wiki/syntheses/second-brain-llm-wiki-operating-model]] - recommended operating model for this vault.

## Questions

No durable question pages yet.

## Maintenance

- Last architecture setup: 2026-06-23.
- Next useful lint pass: after the first 5-10 ingested sources.
